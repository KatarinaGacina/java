...
public class LinkedListIndexedCollection<E> implements List<E> {
	...
	private static class ListNode<E> {
		...
		/**
		 * element stored in the list node
		 */
		private E value;
	}
	...
	public E get(int index) {
		...
		ListNode<E> cvor = this.pronadiIndeks(index);
	 ...
	 */
	private ListNode<E> pronadiIndeks(int index) {
		int pom = this.size / 2;
		ListNode<E> cvor;
	...
	/**
	 * Private static class which represents ElementGetter<E> for class LinkedListIndexedCollection.
	 * @author Katarina Gacina
	 */
	private static class ElementsGetterLinkedList<E> implements ElementsGetter<E> {
		 ...
		 */
		LinkedListIndexedCollection<E> a;
		
		/**
		 * current list node
		 */
		private ListNode<E> trenutniCvor;
		/**
		 ...
		 */
		public ElementsGetterLinkedList(LinkedListIndexedCollection<E> a) {
		...
		public E getNextElement() {
			...
			E element = this.trenutniCvor.value;
	...
	public <T> T[] toArray(T[] a) {
		...
		if (a.length < this.size) {
			a = (T[]) Arrays.copyOf(a, this.size);
		}
		
        int i = 0;
        for (ListNode<E> node = this.first; node != null; node = node.next)
            a[i++] = (T) node.value;
